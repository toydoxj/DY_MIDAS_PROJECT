...
export default function MaterialSection({ storyRows }: { storyRows: StoryRow[] }) {
  ...
  const thCls = "px-2 py-1.5 text-center text-xs font-semibold text-gray-400";
  const tdCls = "px-1.5 py-1 text-center text-sm";
  const selectCls = "w-full rounded bg-gray-700/50 border border-gray-600 px-1.5 py-1 text-sm text-gray-200 text-center focus:outline-none focus:ring-1 focus:ring-emerald-500";
    ...
    <SectionCard as="form" title="재료 강도" onSubmit={handleSave}>
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          <span className="text-xs font-medium text-gray-400">콘크리트</span>
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          <span className="text-xs font-medium text-gray-400">철근</span>
          <Button type="button" variant="ghost" size="xs"
            ...
            <tbody>
              {data.rebar.map((row, i) => (
                <tr key={i} className="border-b border-gray-700/30">
                  ...
                  <td className={`${tdCls} text-gray-400`}>{row.strength}</td>
                  <td className={tdCls}>
                    {row.dia_dir === "전부재" ? (
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          <span className="text-xs font-medium text-gray-400">강재</span>
          <Button type="button" variant="ghost" size="xs"
        ...
        {data.steel.length > 0 ? (
          ...
          <p className="text-xs text-gray-400 px-2">강재 없음 — 위 버튼으로 추가</p>
        )}
      </div>
