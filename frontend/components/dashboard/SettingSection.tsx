          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs text-left">
              <thead><tr className="border-b border-gray-200">
                <th className="pb-2 pr-4 font-medium text-gray-500">Load Case</th>
                <th className="pb-2 pr-4 font-medium text-gray-500 text-right">Factor</th>
                <th className="pb-2 font-medium text-gray-500 text-center w-10"></th>
              </tr></thead>
              <tbody>
                {rows.map((r) => (
                  <tr key={r.id} className="border-b border-gray-100 hover:bg-gray-50">
                    <td className="py-1.5 pr-4 text-white">{r.LCNAME}</td>
                    <td className="py-1.5 pr-4 text-gray-700 text-right">{r.factor !== null ? r.factor : "-"}</td>
                    <td className="py-1.5 text-center text-lg">{r.valid ? <span className="text-emerald-600">●</span> : <span className="text-red-500">●</span>}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <>
            <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
              <div className="flex items-center justify-between">
                <span className="text-gray-500">Mass Direction</span>
                <div className="flex gap-1.5">
                  {(["X", "Y", "Z"] as const).map((d) => (
                    <span key={d} className={ltomDat[`DIR_${d}`] ? "text-emerald-600" : "text-gray-500"}>
                      {ltomDat[`DIR_${d}`] ? "\u2713" : "\u2717"}{d}
                    </span>
                  ))}
                </div>
              ...
              {([["bNODAL", "Nodal Load"], ["bBEAM", "Beam Load"], ["bFLOOR", "Floor Load"], ["bPRES", "Pressure"]] as const).map(([key, label]) => (
                <div key={key} className="flex items-center justify-between">
                  ...
                  <span className={ltomDat[key] ? "text-emerald-600" : "text-gray-500"}>{ltomDat[key] ? "\u2713" : "\u2717"}</span>
                </div>
              ))}
            </div>
            {ltomDat.vLC.length > 0 && (
              <div className="mt-2">
                <table className="w-full text-xs text-left">
                  <thead><tr className="border-b border-gray-200">
                    <th className="pb-1 pr-4 font-medium text-gray-500">Load Case</th>
                    <th className="pb-1 font-medium text-gray-500 text-right">Scale Factor</th>
                  </tr></thead>
                  <tbody>
                    {ltomDat.vLC.map((lc, i) => (
                      <tr key={i} className="border-b border-gray-100">
                        <td className="py-1 pr-4 text-white">{lc.LCNAME}</td>
                        <td className="py-1 text-gray-700 text-right">{lc.FACTOR}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
